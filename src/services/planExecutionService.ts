  ...
  private checkReviewGateAfterItem(itemId: string): void {
    ...
    if (reviewStep) {
      ...
      if (this.callbacks.onReviewGateReached) {
        ...
        const hasSuggestions = this.settings.suggestedPrompts && this.settings.suggestedPrompts.length > 0;
        const gateMessage = `I've completed ${doneCount} of ${totalCount} steps. I'll need your approval on: **${reviewStep.description}**. You can review the deliverables above and approve when ready.${hasSuggestions ? '\n\nIn the meantime, here are some other things I can help with:' : ''}`;
